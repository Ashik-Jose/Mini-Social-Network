...
const PostCard = ({post}) => {
   ...
   function arrayBufferToBase64( buffer ) {
    ...
    return (
      ...
      <Card.Img variant="top" src={`data:image/jpg;base64,${arrayBufferToBase64(post.postPic.data)}` || Placeholder} style={{height:"20rem",width:"100%",padding:"5px",borderRadius:"6%",opacity:loading ? "0.2":"1"}} />
      <Card.Body>
      ...
      <i class="bi bi-trash" onClick={()=>{
        setLoading(true);
        API.post('/profile/friend/deleteposts/'+`${userid}`+'?postid='+`${post._id}`).then((response) => {
            setLoading(false);
